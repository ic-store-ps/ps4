...
let uio_iov_addr = undefined;
let dummy_addr = undefined;
...
function init() {
  ...
  const msg_uio_addr = mem.alloc(iovec.sizeof * NUM_MSG_IOV);
  msg_iov = iovec.new(msg_iov_addr);
  msg_uio = uio.new(msg_uio_addr);
...
function cleanup() {
  ...
  if (uio_iov_addr !== undefined) {
    mem.free(uio_iov_addr);
    uio_iov_addr = undefined;
  }
  if (dummy_addr !== undefined) {
    mem.free(dummy_addr);
    dummy_addr = undefined;
  }
