...
class RPCWorker {
  constructor(name) {
    ...
    this.worker.onerror = (e) => {
      logger.error(`Worker ${this.name} error: ${e.message}`);
      for (const [, promise] of this.promises) {
        promise.reject(new Error(`Worker ${this.name} crashed: ${e.message}`));
      }
      this.promises.clear();
    };
  }
  ...
  terminate() {
    for (const [, promise] of this.promises) {
      promise.reject(new Error(`Worker ${this.name} terminated`));
    }
    this.promises.clear();
  ...
  async init() {
    ...
    const marker_buf_data = marker_arr.buffer;
