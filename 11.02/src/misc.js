...
class BInt {
  ...
  mul(value) {
    ...
    const MASK = 0xFFFF;
    const a0 = this.lo & MASK, a1 = this.lo >>> 16;
    const a2 = this.hi & MASK, a3 = this.hi >>> 16;
    const b0 = value.lo & MASK, b1 = value.lo >>> 16;
    const b2 = value.hi & MASK, b3 = value.hi >>> 16;

    const r0 = a0 * b0;
    const r1 = (r0 >>> 16) + a1 * b0 + a0 * b1;
    const r2 = (r1 >>> 16) + a2 * b0 + a1 * b1 + a0 * b2;
    const r3 = (r2 >>> 16) + a3 * b0 + a2 * b1 + a1 * b2 + a0 * b3;

    const lo = ((r1 & MASK) << 16) | (r0 & MASK);
    const hi = ((r3 & MASK) << 16) | (r2 & MASK);
